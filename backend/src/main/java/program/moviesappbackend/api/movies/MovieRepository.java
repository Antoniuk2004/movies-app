...
public class MovieRepository {
    ...
    public Optional<Movie> findById(int id, String username) {
        String sql = "SELECT R.rating, S.name AS studio, C.name AS origin_country, " +
                "MR.rating_name AS mpaa_rating, WSO.name AS watching_status, " +
                "M.movie_id, M.title, M.duration, M.description, M.cover, " +
                "M.release_year, M.movie_path, M.added_at, M.banner " +
                "FROM movies M " +
                ...
                "      FROM users U  " +
                "      INNER JOIN users_watching_statuses UWS ON UWS.user_id = U.user_id " +
